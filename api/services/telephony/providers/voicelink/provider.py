...
class VoiceLinkProvider(TelephonyProvider):
    ...
    async def initiate_call(
        ...
        **kwargs: Any,
        ...
        organization_id = kwargs.get("organization_id") or kwargs.get("user_id")
        user_id = kwargs.get("user_id") or organization_id
        if workflow_id is None or organization_id is None or workflow_run_id is None:
            raise ValueError(
                "VoiceLink initiate_call requires workflow_id, organization_id and "
    ...
    async def validate_phone_number(self, address: str) -> ProviderSyncResult:
        """Check that address belongs to this VoiceLink provider configuration."""
